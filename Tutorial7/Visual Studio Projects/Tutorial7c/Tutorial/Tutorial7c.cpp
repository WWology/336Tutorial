...
GLuint g_edgeThicknessIndex = 0;
GLuint g_numberOfThresholdsIndex = 0;
GLuint g_specularCutoffIndex = 0;
...
GLfloat g_edgeThickness;
GLint g_numberOfThresholds;
GLfloat g_specularCutoff;
...
static void init(GLFWwindow* window)
{
	...
	g_shaderProgramID = loadShaders("PerFragLightingVS.vert", "ToonShadingFS.frag");
	...
	g_edgeThicknessIndex = glGetUniformLocation(g_shaderProgramID, "uEdgeThickness");
	g_numberOfThresholdsIndex = glGetUniformLocation(g_shaderProgramID, "uNumOfThresholds");
	g_specularCutoffIndex = glGetUniformLocation(g_shaderProgramID, "uSpecularCutoff");
	...
	g_material.ambient = glm::vec3(0.2f, 0.2f, 0.2f);
	g_material.diffuse = glm::vec3(0.2f, 0.5f, 1.0f);
	g_material.specular = glm::vec3(0.2f, 0.7f, 1.0f);
	g_material.shininess = 40.0f;

	// initialise toon shading properties
	g_edgeThickness = 0.2f;
	g_numberOfThresholds = 4;
	g_specularCutoff = 0.5f;
...
static void render_scene()
{
	...
	glUniform1i(g_lightTypeIndex, g_light.type);
	...
	glUniform1f(g_edgeThicknessIndex, g_edgeThickness);
	glUniform1i(g_numberOfThresholdsIndex, g_numberOfThresholds);
	glUniform1f(g_specularCutoffIndex, g_specularCutoff);
...
int main(void)
{
	...
	// create toon shading entries
	TwAddVarRW(TweakBar, "Edge", TW_TYPE_FLOAT, &g_edgeThickness, " group='Toon Shading' min=0.0 max=0.4 step=0.01");
	TwAddVarRW(TweakBar, "Thresholds", TW_TYPE_INT32, &g_numberOfThresholds, " group='Toon Shading' min=1 max=10 step=1");
	TwAddVarRW(TweakBar, "Cutoff", TW_TYPE_FLOAT, &g_specularCutoff, " group='Toon Shading' min=0.0 max=1.0 step=0.01");

	// create material properties entries
	TwAddVarRW(TweakBar, "Ambient", TW_TYPE_COLOR3F, &g_material.ambient[0], " group='Material' ");
	TwAddVarRW(TweakBar, "Diffuse", TW_TYPE_COLOR3F, &g_material.diffuse[0], " group='Material' ");
	TwAddVarRW(TweakBar, "Specular", TW_TYPE_COLOR3F, &g_material.specular[0], " group='Material' ");
